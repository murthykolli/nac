...
<html>    
    ...
    $(document).ready(function(){
        $("#insured_list tr").not(':first').hover(
    		    function () {
    		    if(!$(this).find("input[name='subAdminDisplayId']").is(':checked')){
    		    ...
    		    $(this).find("input[name='subAdminDisplayId']").prop("checked",true);
    		    });
    });
    ...
    element.innerHTML = pagerHtml;
        ...
        <div class="adm_new_cl_head_txt_dsp" align="left">SUB ADMINS LIST</div>
        </div>
        <div class="col-md-5 col-sm-5">
        <div class="adm_new_cl_sub_txt_dsp" align="right">Total sub admins: <b id="displayCount" class="new_clients_no_dsp"></b></div>
        </div>
        ...
        <form action="subAdminDeleteFromDB" name="subAdminDelete" id="subAdminDeleteId" method="get">      
        <div class="row top_adjustment_10">
        <div class="col-md-1 col-sm-1">
        </div> <!-- /.col-md-1 -->        
        ...
        <display:table  id="insured_list"  class="tablesorter" name="subAdminDisplayList" style="margin-top: 0px; margin-left: 0px;">
        <display:column headerClass="sortDisabled"><label>            
        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="subAdminDisplayId" value="${insured_list.userId}"/><span></span></label>
        </display:column>
        <display:column property ="createdDate" title="START DATE" format="{0,date,MM/dd/yyyy}" />
        <display:column property ="firstName" title="SUB ADMIN NAME"/>
        <display:column property ="userName" title="SUB ADMIN ID"/>
        </display:table>
		...
		<input type="hidden" name="subAdminDeleteRef"/> 		            
		</div> <!-- /.col-md-10 -->
		<div class="col-md-1 col-sm-1">
        </div> <!-- /.col-md-1 -->
		...
		<h4><input class="dash_board_back_submit_Btn" style="margin-right: -12px; margin-top: 2px;" onclick="return subAdminBack();" type="image" title="BACK" src="resources/images/backButton.png"></h4>
		</div>
		
		<div class="col-md-2 col-sm-2 text-left">
		<h4><button class="admin_cl_dir_submit_Btn" id="viewProfileId" onclick="return deleteSubAdmin();">DELETE</button></h4>
		</div>
		
		<div class="col-md-4 col-sm-4 text-left">
		<h4><button class="admin_cl_dir_submit_Btn" id="viewProfileId" onclick="return newSubAdmin();">CREATE SUB ADMIN</button></h4>
		</div>
        ...
        var count = '<c:out value="${subAdminDisplayListCount}"/>';
        if(count > 0) {
        ...
        function subAdminBack(){
	        document.subAdminDelete.subAdminDeleteRef.value = "Sub Admin Back";	        
        }

        function newSubAdmin(){
	        document.subAdminDelete.subAdminDeleteRef.value = "New Sub Admin";	        
        }

        function deleteSubAdmin(){
	        document.subAdminDelete.subAdminDeleteRef.value = "Delete Sub Admin";
	        var subAdminDisplayIds="";        
		        for(var k=0; k < document.subAdminDelete.subAdminDisplayId.length; k++) {
			        if(document.subAdminDelete.subAdminDisplayId[k].checked){
			        	subAdminDisplayIds=document.subAdminDelete.subAdminDisplayId[k].value;        
			        }
		        }
	    
	        if(document.subAdminDelete.subAdminDisplayId.checked){
	        	subAdminDisplayIds=document.subAdminDelete.subAdminDisplayId.value;
	        }
	        
	        if(subAdminDisplayIds === ""){
