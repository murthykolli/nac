...
<html>    
    ...
    $(document).ready(function(){    	        
        $("#insured_list tr").not(':first').hover(
    		    function () {
    		    if(!$(this).find("input[name='userIdRef']").is(':checked')){
    		    ...
    		    $(this).find("input[name='userIdRef']").prop("checked",true);
    		    });
    });
    ...
    element.innerHTML = pagerHtml;
        ...
        <div class="adm_new_cl_head_txt_dsp" align="left">DESIGN LOGIN INFO</div>
        </div>
        <div class="col-md-5 col-sm-5">
        ...
        <display:table  id="insured_list"  class="tablesorter" name="adminPackageSavingList" style="margin-top: 0px; margin-left: 0px;">
        <%-- <display:column headerClass="sortDisabled"><label>            
        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="userIdRef" value="${insured_list.userId}"/><span></span></label>
        </display:column> --%>
        
        <display:column property ="lastName" title="INDEX"/>
        <display:column style="text-transform: uppercase;" property ="firstName" title="NAME"/>        
        <display:column property ="userName" title="EMAIL"/>        
        ...
        var count = '<c:out value="${adminPackageListCount}"/>';        
        ...
        function profile(){
	        var userIdRefs="";        
		        for(var k=0; k < document.designerClients.userIdRef.length; k++) {
			        if(document.designerClients.userIdRef[k].checked){
			        	userIdRefs=document.designerClients.userIdRef[k].value;        
			        }
		        }
	    
	        if(document.designerClients.userIdRef.checked){
	        	userIdRefs=document.designerClients.userIdRef.value;
	        }
