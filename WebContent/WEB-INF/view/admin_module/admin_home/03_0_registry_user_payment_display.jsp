...
<html>    
    ...
    $(document).ready(function(){    	        
        $("#insured_list tr").not(':first').hover(
    		    function () {
    		    if(!$(this).find("input[name='regPaymentUserIdRef']").is(':checked')){
    		    ...
    		    $(this).find("input[name='regPaymentUserIdRef']").prop("checked",true);
    		    });
    });
    ...
    element.innerHTML = pagerHtml;
        ...
        <div class="adm_new_cl_head_txt_dsp" align="left">REGISTRY PAYMENT INFO</div>
        </div>
        <div class="col-md-5 col-sm-5">
        ...
        <form action="adminViewRegistryUserPayment" name="regiPaymentInfo" id="regiPaymentInfoId" method="get">      
        ...
        <display:table  id="insured_list"  class="tablesorter" name="adminRegistryUserPaymentSavingList" style="margin-top: 0px; margin-left: 0px;">
        <display:column headerClass="sortDisabled"><label>            
        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="regPaymentUserIdRef" value="${insured_list.registrantPaymentInfoSavingId}"/><span></span></label>
        </display:column>
        
        <display:column property ="status" title="INDEX"/>
        <display:column style="text-transform: uppercase;" property ="billAddress1" title="NAME"/>        
        <display:column property ="billAddress2" title="EMAIL"/>
		...
		<h4><button class="admin_cl_dir_submit_Btn" id="viewRegUserPaymentId" onclick="return viewRegUserPayment();">VIEW PAYMENT INFO</button></h4>
		</div>
        ...
        var count = '<c:out value="${adminRegistryUserPaymentListCount}"/>';        
        ...
        function viewRegUserPayment(){
	        var regPaymentUserIdRefs="";        
		        for(var k=0; k < document.regiPaymentInfo.regPaymentUserIdRef.length; k++) {
			        if(document.regiPaymentInfo.regPaymentUserIdRef[k].checked){
			        	regPaymentUserIdRefs=document.regiPaymentInfo.regPaymentUserIdRef[k].value;        
			        }
		        }
	    
	        if(document.regiPaymentInfo.regPaymentUserIdRef.checked){
	        	regPaymentUserIdRefs=document.regiPaymentInfo.regPaymentUserIdRef.value;
	        }
	        
	        if(regPaymentUserIdRefs === ""){
	        	window.alert("Please select a registry payment");
	        return false;
	        } 
        }
