...
<html>    
    ...
    $(document).ready(function(){    	        
        $("#insured_list tr").not(':first').hover(
    		    function () {
    		    if(!$(this).find("input[name='guestPaymentUserIdRef']").is(':checked')){
    		    ...
    		    $(this).find("input[name='guestPaymentUserIdRef']").prop("checked",true);
    		    });
    });
    ...
    element.innerHTML = pagerHtml;
        ...
        <div class="adm_new_cl_head_txt_dsp" align="left">GUEST PAYMENT INFO</div>
        </div>
        <div class="col-md-5 col-sm-5">
        <div class="adm_new_cl_sub_txt_dsp" align="right">Total payments: <b id="displayCount" class="new_clients_no_dsp"></b></div>
        </div>
        ...
        <form action="adminViewGuestUserPayment" name="guestPaymentInfo" id="guestPaymentInfoId" method="get">      
        ...
        <display:table  id="insured_list"  class="tablesorter" name="adminGuestUserPaymentSavingList" style="margin-top: 0px; margin-left: 0px;">
        <display:column headerClass="sortDisabled"><label>            
        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="guestPaymentUserIdRef" value="${insured_list.guestUserPaymentInfoSavingId}"/><span></span></label>
        </display:column>
        
        <display:column property ="status" title="INDEX"/>
        <display:column style="text-transform: uppercase;" property ="billAddress1" title="NAME"/>        
        <display:column property ="guestEmail" title="GUEST EMAIL"/>
		...
		<h4><button class="admin_cl_dir_submit_Btn" id="viewGuestUserPaymentId" onclick="return viewGuestUserPayment();">VIEW PAYMENT INFO</button></h4>
		</div>
        ...
        var count = '<c:out value="${adminGuestUserPaymentListCount}"/>';        
        ...
        function viewGuestUserPayment(){
	        var guestPaymentUserIdRefs="";        
		        for(var k=0; k < document.guestPaymentInfo.guestPaymentUserIdRef.length; k++) {
			        if(document.guestPaymentInfo.guestPaymentUserIdRef[k].checked){
			        	guestPaymentUserIdRefs=document.guestPaymentInfo.guestPaymentUserIdRef[k].value;        
			        }
		        }
	    
	        if(document.guestPaymentInfo.guestPaymentUserIdRef.checked){
	        	guestPaymentUserIdRefs=document.guestPaymentInfo.guestPaymentUserIdRef.value;
	        }
	        
	        if(guestPaymentUserIdRefs === ""){
	        	window.alert("Please select a guest payment");
	        return false;
	        } 
        }
