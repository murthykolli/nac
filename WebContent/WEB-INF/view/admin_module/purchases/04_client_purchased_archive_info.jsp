...
<html>    
    ...
    $(document).ready(function(){
        ...
        $("#insured_list tr").not(':first').hover(
    		    function () {
    		    if(!$(this).find("input[name='shopingCartPaymentRef']").is(':checked')){
    		    ...
    		    $(this).find("input[name='shopingCartPaymentRef']").prop("checked",true);
    		    $('.ordStatusCl:contains("YES")').css('color', 'green');
    		    $('.ordStatusCl:contains("NO")').css('color', 'red');
    			});    		   
    });
    ...
    element.innerHTML = pagerHtml;
        ...
        <form action="archivePurchaseDataUpdate" name="designerClients" id="designerClientsId" method="get">     
        ...
        <display:table  id="insured_list"  class="tablesorter" name="archiveInfoSavingList" style="margin-top: 0px; margin-left: 0px;">
        <display:column headerClass="sortDisabled"><label>            
        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="shopingCartPaymentRef" value="${insured_list.shoppingCartPaymentInfoSavingId}"/><span></span></label>
        </display:column>
		...
		<a href="admnPurchases"><img class="back_submit_Btn" title="BACK" style="margin-right: -7px;" src="resources/images/backButton.png"></a>
		</div>
		...
		<div class="col-md-2 col-sm-2 text-left">
		<h4><button class="admin_archive_submit_Btn" id="viewArchiveId" onclick="return clientDeleteInfo();">&nbsp;DELETE&nbsp;</button></h4>
		</div>
		
		<div class="col-md-4 col-sm-4">
        </div> <!-- /.col-md-4 -->        
        </div> <!-- /.row -->
        <input type="hidden" name="archiveInfoReffName"/>
        </form>
        ...
        var count = '<c:out value="${archiveInfoListCount}"/>';        
        ...
        function clientDeleteInfo(){
	        document.designerClients.archiveInfoReffName.value = "Archive Client Delete";
	        var shopingCartPaymentRefs="";
	        
		        for(var k=0; k < document.designerClients.shopingCartPaymentRef.length; k++) {
			        if(document.designerClients.shopingCartPaymentRef[k].checked){
			        	shopingCartPaymentRefs=document.designerClients.shopingCartPaymentRef[k].value;        
			        }
		        }
	    
	        if(document.designerClients.shopingCartPaymentRef.checked){
	        	shopingCartPaymentRefs=document.designerClients.shopingCartPaymentRef.value;
	        }
	        
	        if(shopingCartPaymentRefs != "") {
            	var where_to= confirm("Do you want to delete selected Product?");
	            if (where_to) {
	            	return true;
	            } else {
	            	return false;
        ...
        function clientArchiveInfo(){
	        document.designerClients.archiveInfoReffName.value = "Archive Client Update";
	        var shopingCartPaymentRefs="";        
		        for(var k=0; k < document.designerClients.shopingCartPaymentRef.length; k++) {
			        if(document.designerClients.shopingCartPaymentRef[k].checked){
			        	shopingCartPaymentRefs=document.designerClients.shopingCartPaymentRef[k].value;        
