...
<html>
	...
	<script type="text/javascript">
 	$(document).ready(function() { var cashFundName1 = "<c:out value="${cashFundName}"/>";
 		$("#addRegCFButtonId").click(function () { var result = true; 	 		
 	 		if(cashFundName1 == "Romantic Dinner" || cashFundName1 == "Wine Tasting") { if($("#cashFundsAmount1Id").val() == "" || $("#cashFundsAmount1Id").val() == 0) { document.regiAddGift.cashFundReqAmount.value = "200"; } else { document.regiAddGift.cashFundReqAmount.value = $("#cashFundsAmount1Id").val(); }
 		...
 		var cashFundsMessage1 = "<c:out value="${cashFundsMessage}"/>"; var cashFundsAmount1 = "<c:out value="${cashFundsAmount}"/>"; $("#cashFundsMessageId").val(cashFundsMessage1);
 		if(cashFundName1 == "Romantic Dinner" || cashFundName1 == "Wine Tasting") { $("#cashFundsAmount1Id").val(cashFundsAmount1); $('#firstFieldsDisplayId').show(); $('#secondFieldsDisplayId').hide(); $('#thiredFieldsDisplayId').hide();
		} else if(cashFundName1 == "Couples Spa Day" || cashFundName1 == "Sunset Sail") { $("#cashFundsAmount2Id").val(cashFundsAmount1); $('#firstFieldsDisplayId').hide(); $('#secondFieldsDisplayId').show(); $('#thiredFieldsDisplayId').hide();
		} else { $("#cashFundsAmount3Id").val(cashFundsAmount1); $('#firstFieldsDisplayId').hide(); $('#secondFieldsDisplayId').hide(); $('#thiredFieldsDisplayId').show(); }
 	 	var words = $('#cashFundNameBigId').text().split(' '); var html = ''; $.each(words, function() { html += '<span class="cash_fund_big_name">'+this.substring(0,1)+'</span>'+this.substring(1) + ' '; }); $('#cashFundNameBigId').html(html);
 	 	var words = $('#cashFundNameSmlId').text().split(' '); var html = ''; $.each(words, function() { html += '<span class="cash_fund_sml_name">'+this.substring(0,1)+'</span>'+this.substring(1) + ' '; }); $('#cashFundNameSmlId').html(html); 	 	
 ...
 <!-- Left Coding Start -->        
		...
		<div align="center" class="row top_adjustment_35"> <img class="cash_fund_image_dsp" src="<c:out value='${cashFundsImgPathDsp}'/>"> </div>
		<div align="center" id="cashFundNameBigId" class="cash_fund_hdtext_dsp top_adjustment_30">${cashFundName}</div> <div align="left" class="cashfuns_mlmtop_adj"> <div class="reg_cash_fund_sb_text_dsp">REQUESTED AMOUNT:</div>
		...
		<div class="reg_cash_fund_sb_text_dsp">NOTE FOR YOUR GUESTS <font class="reg_cash_fund_opt_text_dsp">(OPTIONAL)</font>:</div> <div><textarea class="cashfuns_textarea_st" maxlength="100" name="cashFundsMessage" id="cashFundsMessageId" onkeyup="countChar(this)" placeholder="TELL YOUR GUESTS WHY YOU WANT THIS ..."></textarea></div> <div id="rowShowId" class="rln_top_mesg_dspl_cl"><div class="rg_billing_bm_num_text_dis cash_fund_pdng_dspl"><font id="incCharNum"></font> / <font id="decCharNum"></font></div></div> <div id="rowHideId"><div class="rg_billing_bm_num_text_dis cash_fund_pdng_dspl">0/100</div></div> </div>
		<div class="row top_adjustment_50"> <div class="col-md-3 col-sm-3" align="right"> 
		<form action="regCashFundBack" method="get"> <h4><input class="back_submit_Btn rfund_box_polt_dsp" type="image" title="BACK" src="resources/images/backButton.png"></h4> </form> </div> <div class="col-md-9 col-sm-9"> <div class="text-left rfund_box_polet_dsp"> 
		<form action="regTopCashFundsInfoUpdating" name="regiAddGift" method="get"> <h4><button class="reg_add_con_submit_Btn_dsp" id="addRegCFButtonId">ADD TO REGISTRY</button></h4> <input type="hidden" id="cashFundNameId" name="cashFundsName" value="${cashFundName}"> <input type="hidden" id="cashFundReqAmountId" name="cashFundReqAmount"> <input type="hidden" id="cashFundReqMessageId" name="cashFundReqMessage"> </form> </div> </div> </div>
...
<!-- Box display coding -->	        
			...
			<div class="row top_adjustment_30"> <div class="col-md-4 col-sm-4"> <div class="row" align="center" style="margin-top: 0px;"> <div><img src="<c:out value='${cashFundsImgPathDsp}'/>" height="100" width="100"></div> </div> </div>
			<div class="col-md-4 col-sm-4 reg_guest_rsbox_red_dvd"> <div align="center" id="cashFundNameSmlId" class="cash_fund_side_hdtext_dsp top_adjustment_38">${cashFundName}</div> </div> <div class="col-md-2 col-sm-2"> <img src="resources/images/registry/reg_guest_img/reg_gift_group_img.png" class="rfund_mgtpml_pl_dsp"> </div>
			...
			<div class="row"> <div class="col-md-12 col-sm-12 reg_guest_rs_bott_box_dsp"> <div align="center" class="cash_fund_gs_bott_box_htext_dsp rfund_height_dsp_cl"><font id="cashFundMessageTextDisplayId"></font></div> </div> </div> </div>			
		    </div> <div class="col-md-1 col-sm-1"></div> </div> </div> </div> </div></div>		
		</body>
</html>