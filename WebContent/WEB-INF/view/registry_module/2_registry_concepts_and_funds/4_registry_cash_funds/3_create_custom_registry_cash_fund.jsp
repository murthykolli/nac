...
<html>
	...
	<script type="text/javascript">
 	$(document).ready(function() { var customFundName1 = "<c:out value="${customFundName}"/>"; $("[name=fundName]").val(customFundName1);
 		var cashFundsMessage1 = "<c:out value="${cashFundsMessage}"/>"; var cashFundsAmount1 = "<c:out value="${cashFundsAmount}"/>"; $("#cashFundsAmountId").val(cashFundsAmount1); $("#cashFundsMessageId").val(cashFundsMessage1); var regCustomFundError1 = "<c:out value="${regCustomFundError}"/>";
 		if(regCustomFundError1 === "Have Custom Name In DB") { alert("A fund with that name already exists. Please rename this fund."); } var textFieldRegExp = /^[A-Za-z 0-9][A-Za-z 0-9 -#/_().&amp;]*$/;
 		$("#addRegCFButtonId").click(function () { var result = true; if($("#fundNameId").val() !== "") { $("#cashFundNameIdLabel").css("color","black"); } else { $("#cashFundNameIdLabel").css("color","red"); result=false; } if($("#cashFundsAmountId").val() == "" || $("#cashFundsAmountId").val() == "0") { document.regiAddGift.cashFundReqAmount.value = "2500"; } else { document.regiAddGift.cashFundReqAmount.value = $("#cashFundsAmountId").val(); } document.regiAddGift.cashFundReqMessage.value = $("#cashFundsMessageId").val(); document.regiAddGift.customFundName.value = $("#fundNameId").val(); return result; });
 	 	...
 	 	var words = $('#cashFundNewNameTextId').text().split(' '); var html = ''; $.each(words, function() { html += '<span class="first_letter_dsp">'+this.substring(0,1)+'</span>'+this.substring(1) + ' '; }); $('#cashFundNewNameTextId').html(html); $("#cashFundsMessageId").on("input", function(){ $("#cashFundMessageTextDisplayId").text($(this).val()); });
 	});
 ...
 <!-- Left Coding Start -->        
		...
		<table> <tr> <td><input value="$" readonly class="cashfuns_inputtag_st"></td> <td><input type="text" name="cashFundsAmount" maxLength="5" placeholder="2500" id="cashFundsAmountId" onkeypress="return numeralsOnly(event);" class="cashfuns_input_txtfld_st"></td> </tr> </table>
		...
		<div class="row top_adjustment_50"> <div class="col-md-3 col-sm-3" align="right"> <form action="createCashFunds" method="get"> <h4><input class="back_submit_Btn rfund_box_polt_dsp" type="image" title="BACK" src="resources/images/backButton.png"></h4> </form> </div>
...
<!-- Box display coding -->	        
		...
		<div id="lorem" class="rln_pop_pad_adj"> <div class="rln_pop_uplph_txt_cl">Upload Cash Fund Image</div><br>
