...
<html>
	...
	<script type="text/javascript">
 	$(document).ready(function() { var regFTContributeAmount = "<c:out value="${regFTCotributedPrice}"/>";
 	 	if(regFTContributeAmount !== "null") { $("#regiFundAmountId").val(regFTContributeAmount); } else { $("#regiFundAmountId").val(""); } var priceReg = /^[0-9\.]+$/;
 		$("#contRegistryButtonId").click(function () { var result = true; 		    
	 	if(priceReg.test($("#regiFundAmountId").val())){ $("#regiFundAmountId").css("border-color","black"); var regiFTCotributedNeedPrice1 = "<c:out value="${regFTCotributedRemainingPrice}"/>"; var regFTCotributedNeedPrice1 = regiFTCotributedNeedPrice1.replace(",",""); var cbCotributedNeedPriceDb = parseFloat(regFTCotributedNeedPrice1); var cbCotributedPriceDb = parseFloat($("#regiFundAmountId").val());	 		
	 	if(cbCotributedPriceDb > cbCotributedNeedPriceDb) { alert("Please make sure the amount you wish to gift is equivalent to or less than the remaining balance of funds requested."); result=false; } } else{ $("#regiFundAmountId").css("border-color","red"); result=false; } return result; });
 	 	...
 	 	var regStillNeedAmount1 = "<c:out value="${regFTCotributedRemainingPrice}"/>"; var regiStillNeedAmount = parseInt(regStillNeedAmount1);
...
<!-- Box display coding -->	        
		...
		<div class="row" align="center" style="margin-top: 0px;"> <div><img src="${regFTCotributedProductImage}" class="reg_cb_contributed_image_dsp"></div> </div> </div>
		<div class="col-md-4 col-sm-4 reg_guest_rsbox_red_dvd"> <div align="center" style="margin-top: 38px;" id="cbontributedNewNameTextId" class="reg_cb_contributed_name_dsp">${regFTCotributedProductName}</div> </div>
		...
		<div class="row top_adjustment_80"> <div class="col-md-4 col-sm-4"> <div align="center" class="reg_guest_fund_rstest_dsp"><p>CONTRIBUTE ANY AMOUNT TOWARDS THE ${regFTCotributedProductName}:</p></div> </div>
		...
		<td><input type="text" name="regFTCotributedPrice" id="regiFundAmountId" maxLength="8" class="reg_guest_rsinput_amount_fld_cl" onkeypress="return numeralsOnly(event,this.id);"></td> </tr> </table>
		<div class="reg_guest_st_need_amt_text" id="regStillNeedAmountId" style="display: none;">STILL NEEDS: $${regFTCotributedRemainingPrice}</div> </div>
