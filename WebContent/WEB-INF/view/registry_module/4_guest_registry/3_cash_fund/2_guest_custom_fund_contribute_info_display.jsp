	...
	<script type="text/javascript">
 	$(document).ready(function() { var cashFundsMessage1 = "<c:out value="${cashFundsMessage}"/>"; var priceReg = /^[0-9]+$/; 		
 		$("#contGuestRegButtonId").click(function () { var result = true; 		    
	 		if(priceReg.test($("#contributeAmountId").val())){ $("#contributeAmountId").css("border-color","black"); var guetCustomFundNeedAmount1 = "<c:out value="${regCashFundsNeedAmount}"/>"; var guestCustomFundNeedAmount1 = guetCustomFundNeedAmount1.replace(",",""); var guestCustomFundNeedAmountDb = parseFloat(guestCustomFundNeedAmount1); var customFundCotributedPriceDb = parseFloat($("#contributeAmountId").val());	 		
	 		if(customFundCotributedPriceDb > guestCustomFundNeedAmountDb) { alert("Please make sure the amount you wish to gift is equivalent or less than the remaining balance of funds requested."); result=false; } } else{ $("#contributeAmountId").css("border-color","red"); result=false; } return result; });
...
<!-- Box display coding -->	        
		...
		<input type="hidden" name="registryCashFundsId" value="${registryCashFundContId}"> <input type="hidden" name="cashFundsName" value="${cashFundNewName}"> <input type="hidden" name="registryCashFundsImage" value="${registryCashFundsImage}"> <input type="hidden" name="guestCashFundsNeedAmount" value="${regCashFundsNeedAmount}"> </form> </div>
