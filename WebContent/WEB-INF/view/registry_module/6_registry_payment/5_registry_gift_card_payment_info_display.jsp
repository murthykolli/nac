...
<html>
	...
	<script type="text/javascript">
 	$(document).ready(function() { 		 
	    ...
	    $("[name=payCardNumber]").val(payCardNumber1); $("[name=payExpMonth]").val(payExpMonth1); $("[name=payExpYear]").val(payExpYear1); $("[name=nameOnCard]").val(nameOnCard1); $("[name=payCvvNumber]").val(payCvvNumber1); var giftCardStatus1 = '<c:out value="${giftCardStatus}"/>';
...
// Package Payment Gift Card validation
        ...
        <div class="sec_rn_gift_code_frm"> <div class="promo-code">
        <div class="sr_pay_promo_text_dsp">GIFT CARD / PROMO CODE:</div>
        <input type="text" name="regGiftCardCouponCode" id="regGiftCardCouponCodeId" maxlength="8" class="pack_pay_promo_and_gift_inpt_box"> </div>
		...
		<tr> <td class="gift_cart_pay_est_totl">TOTAL:</td> <td class="gift_pay_price_shp_tol">$${shoppingCartTotalAmountInSn}&nbsp;</td> </tr>
		...
		<input type="hidden" name="payCardNumber"/> <input type="hidden" name="payExpMonth"/> <input type="hidden" name="payExpYear"/> <input type="hidden" name="payCvvNumber"/> <input type="hidden" name="nameOnCard"/> <input type="hidden" name="giftCardPrice" value="${giftCardPrice}"/> <input type="hidden" name="totalPaybleAmount" value="${totalPaybleAmount}"/> <input type="hidden" name="regGiftCardCouponCode" value="${regGiftCardCouponCode}"/>
		</form>
		</div> </div> </div> </div>	     	
