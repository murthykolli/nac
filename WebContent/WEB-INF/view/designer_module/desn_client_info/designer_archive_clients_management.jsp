...
<html>    
    ...
    $(document).ready(function(){
    	$('#insured_list tr').each(function() {  		
    		$('.messageCl:contains("YES")').css('color', 'red');
    		$('.newAlertCl:contains("YES")').css('color', 'red');		
    	});
    	
        $("#insured_list tr").not(':first').hover(
    		    function () {
    		    if(!$(this).find("input[name='desArchiveCliens']").is(':checked')){
    		    ...
    		    $(this).find("input[name='desArchiveCliens']").prop("checked",true);
    		    $('.messageCl:contains("YES")').css('color', 'red');
        		$('.newAlertCl:contains("YES")').css('color', 'red');        		
    		    });
    ...
    element.innerHTML = pagerHtml;
        ...
        <form action="designerClArchiveDataUpdate" name="designerClients" id="designerClientsId" method="get">
        ...
        <display:table id="insured_list" class="tablesorter" name="designerClArchiveSavingList">
        <display:column headerClass="sortDisabled"><label>            
        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="desArchiveCliens" value="${insured_list.clientInfoSavingId}, ${insured_list.designerInfoSavingId}"/><span></span></label>
        </display:column>
		...
		<h4><button class="management_submit_Btn" style="margin-left: -7px;" id="viewClientId" onclick="return archive();">ARCHIVE</button></h4>
		</div>
        ...
        var count = '<c:out value="${designerClArchiveSavingCount}"/>';
        ...
        function archive(){
	        var desArchiveClienss="";        
		        for(var k=0; k < document.designerClients.desArchiveCliens.length; k++) {
			        if(document.designerClients.desArchiveCliens[k].checked){
			        	desArchiveClienss=document.designerClients.desArchiveCliens[k].value;        
			        }
		        }
	    
	        if(document.designerClients.desArchiveCliens.checked){
	        	desArchiveClienss=document.designerClients.desArchiveCliens.value;
	        }
	        
	        if(desArchiveClienss === ""){
