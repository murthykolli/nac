...
// Radio Button Value Hold Functionality	    
        ...
        if(designerRegistryRoom1) {
        	$('.radio_container').removeAttr('style');
        	var $radio = $('input[name=typeOfRoomInReg][value="' + designerRegistryRoom1 + '"]').prop('checked', true).trigger('change');
        	$radio.closest('.radio_container') .css('color','#36669b');
        	$radio.closest('.radio_container') .css('text-decoration','underline');
        	$("#radioLabelNalId").html(designerRegistryRoom1);	                
        }

// Check Box Value Hold Functionality        
	    var designerRegistryStyle1 = "<c:out value="${designerRegistryStyle}"/>";
	    if(designerRegistryStyle1) {
        	var checkboxList = [];
        	var checkboxListVal = designerRegistryStyle1.split(",");
        	$('.checkbox_container').removeAttr('style');       	
	    ...
	    if(designerRegistryPrice1) {
        	var checkboxList1 = [];
        	var checkboxListVal1 = designerRegistryPrice1.split(",");
        	$('.checkbox_container_for_price').removeAttr('style');       	
	    ...
	    if(designerRegistryStyle1 !== "" && designerRegistryPrice1 !== "") {
        	$("#bothDisplayId").show();
        } else {
        	$("#bothDisplayId").hide();
        }
	...
	function designRegImageUpload(admnUploadImg) {
	...
	function typeOfStyle() { 
        ...
        document.admStyleSelect.designerRegistryStyle.value = checkboxList;
	...
	function typeOfPrice() { 
        ...
        document.admPriceSelect.designerRegistryPrice.value = checkboxList;
	...
	function typeOfFinishingTouches(finishingTouches) { 		
        ...
        <div class="container">
		        ...
		        <div class="col-md-9 col-sm-9">
		        	...
		        	<div class="row" style="margin-bottom: 25px;">
			        	<div class="col-md-12 col-sm-12">
					        ...
					        <td style="width: 170px; white-space: nowrap;"><font id="radioLabelNalId"></font></td> <td style="width: 570px;"><font id="checkBoxStyleLabelValId"></font> <font style="display: none; font: 1.3em/1.25em Georgia;" id="bothDisplayId">, </font> <font id="checkBoxPriceLabelValId" style="font: 1.3em/1.25em Georgia;"></font></td> <td align="right" style="width: 120px;"><font style="font: bold 1.25em/0.9em Georgia;">${designerRegTotalResultCount}</font> <font style="text-transform: capitalize;">Results</font></td>
					        </tr></table>
					...
					<% if (null != session.getAttribute("designerRegConBdRoomIdsListInSn")) {
					   @SuppressWarnings("rawtypes")
				       ArrayList desnRegConBdRoomIdsListInSn = (ArrayList) session.getAttribute("designerRegConBdRoomIdsListInSn");
				       if (null != desnRegConBdRoomIdsListInSn && desnRegConBdRoomIdsListInSn.size() > 0) {
						   
					   if (null != session.getAttribute("designerRegConBdRoomImagesListInSn")) {
					   @SuppressWarnings("rawtypes")
			           ArrayList desnRegConBdRoomImagesListInSn = (ArrayList) session.getAttribute("designerRegConBdRoomImagesListInSn");
					   if (null != desnRegConBdRoomImagesListInSn && desnRegConBdRoomImagesListInSn.size() > 0) {						   
			    	%>
					 
					<div class="row" style="max-height: 785px; overflow-y: scroll;">
					<% for(int i=0; i<desnRegConBdRoomIdsListInSn.size(); i++) { int count = (i + 1);%>
			        	...
			        	<div class="reg_dsn_curated_cb_rs_imgs_dsp">
				        	...
				        	<a class="example-image-link" onClick = "designRegImageUpload(<%=desnRegConBdRoomIdsListInSn.get(i)%>);" style="cursor: pointer;" >
				        	<img src="data:image/jpg;base64,<%=desnRegConBdRoomImagesListInSn.get(i)%>" align="middle" id="<%=desnRegConBdRoomIdsListInSn.get(i)%>" class="image_disply_proper_cl">
				        	</a>
				        		<div class="middle">
	        ...
	        <form action="designerSelectedRoomAndStyleRed" id="roomImgsFormId" name="admRoomSelect" method="get">
	        ...
	        <form action="designerSelectedStyleAndRoomRed" id="styleImgsFormId" name="admStyleSelect" method="get">
	        ...
	        <form action="designerSelectedStyleRoomAndPriceRed" id="priceImgsFormId" name="admPriceSelect" method="get">
                <input type="hidden" id="designerRegistryRoomId" name="designerRegistryRoom" value="${designerRegistryRoom}">
                <input type="hidden" id="designerRegistryStyleId" name="designerRegistryStyle" value="${designerRegistryStyle}">
                <input type="hidden" id="designerRegistryPriceId" name="designerRegistryPrice">                 
            </form>
                        
	        <form action="designerUnCheckStyleAndPriceBoxesRed" id="unCheckStyleAndPriceFormId" method="get">
                <input type="hidden" id="designerRegistryRoomId" name="designerRegistryRoom" value="${designerRegistryRoom}">                                
            </form>
            
			<form action="designerSelectedFinishingTouchesRed" id="admSelectedFTId" name="admSelectedFT" method="POST">
                <input type="hidden" id="designerRegistryRoomId" name="designerRegistryRoom" value="${designerRegistryRoom}">
                <input type="hidden" id="designerRegistryStyleId" name="designerRegistryStyle" value="${designerRegistryStyle}">
                <input type="hidden" id="designerRegistryPriceId" name="designerRegistryPrice" value="${designerRegistryPrice}">
        ...
        <form action="designerSelectedRoomAndStyleImageUpload" id="editImgsFormSubmitId" method="get">		
		<input type="hidden" id="designerRegistryRoomId" name="designerRegistryRoom" value="${designerRegistryRoom}">
